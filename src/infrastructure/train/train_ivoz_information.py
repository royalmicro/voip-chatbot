...
from application.models import PredictInformationModel
from application.services import STOPWORDS_ES, SYMBOLS, ParseIntents, SaveVocabulary


class TrainIvozInformation:
    def __init__(self) -> None:
        self.config = Config()
        self.model = PredictInformationModel(self.config.MODEL_INFORMATION)
    ...
    def execute(self) -> None:
        ...
        intents = self.intent_parser.execute(
            json_intents, self.config.MODEL_INFORMATION
        )
        ...
        self.save_vocabulary.execute(all_words, tags, self.config.MODEL_INFORMATION)
